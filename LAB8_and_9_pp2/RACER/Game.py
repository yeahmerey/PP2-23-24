...
#try_again = font.render("Try again , you \n are playing well over time" , True , WHITE)
...
#class for coin
class coin_dollar(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.image = coin_dollar_image
        self.rect = self.image.get_rect()
        self.rect.center = (random.randint(40 , SCREEN_WIDTH - 40) , 0)
    #generate 1st coin randomly
    def move(self):
        global dollar
...
class coin_ruble(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
...
while process:
    ...
    #level_show = font_small.render(f"LEVEL:{level}", True , BLACK)
    #DISPLAYSURF.blit(level_show , (150 , 10))
    ...
    #         pygame.quit()
    #         sys.exit()
    #To be run if collision occurs between Player and Coin_Dollar
    if pygame.sprite.spritecollideany(P1, coin):
        ...
        # if coin_score % hole_standart == 0:
        #     hole = Hole()
        #     holes.add(hole)
        #     all_sprites.add(hole)
    #To be run if collision occurs between Player and Coin_ruble
    ...
    if pygame.sprite.spritecollideany(P1, enemies):
        ...
        DISPLAYSURF.fill(RED)
        #DISPLAYSURF.blit(try_again , (20 ,250))
        DISPLAYSURF.blit(game_over, (30, 250))
